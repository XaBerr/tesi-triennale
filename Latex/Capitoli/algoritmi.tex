...
L'algoritmo \textit{discrete time simulate quantum annealing} (DT-SQA)\cite{QVC} è spesso conosciuto con il nome di \textit{single spin flip Monte Carlo} o anche come \textit{path integral Monte Carlo}\cite{PIMC} è un'algorimto che cerca di ottenere il valore minimo di energia per IM attraverso a delle modifiche locali della discrettizzazione della dinamica di evoluzione degli spin nel tempo.
...
Il \textit{continuos time simulate quantum annealing} (CT-SQA)\cite{QVC} anche chiamato \textit{Riger-Kawashima}\cite{CTSQA} è una variante del Monte Carlo a cluster \textit{Swendsen-Wang}\cite{NCD} ed è un'algorimto che cerca di ottenere il valore minimo di energia per IM attraverso a delle modifiche non locali di raggruppamenti in cluset dei segmenti continui della dinamica di evoluzione degli spin nel tempo.
