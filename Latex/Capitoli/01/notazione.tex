...
Nella notazione braket ci sono due simboli che ricordano l'utilizzo delle parentesi il cui termine inglese è appunto brackets. Il primo simbolo si chiama \myindex{bra} $\langle m |$ ed indica il complesso coniugato della funzione $\psi_m$.
$$\langle m | = \psi_m^{\star}$$
Il secondo simbolo si chiama \myindex{ket} $| n \rangle$ ed indica la funzione stessa $\psi_n$.
$$| n \rangle = \psi_n$$
...
La \myindex{condizione di ortonormalità}, ovvero, dove una famiglia di funzioni $\psi_n$ si dice ortonormale se rispetta la seguente proprietà
$$\langle m | n \rangle = \int \psi_m^{\star} \psi_n\, d\tau = \delta_{mn}$$
dove $\delta_{mn}$ si chiama \myindex{delta di Kronecker} e vale $1$ se $m = n$ mentre vale $0$ nel resto dei casi.
