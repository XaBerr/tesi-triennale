...
\begin{quote}
\index{Postulato 3}\textbf{Postulato 3}: Quando un sistema è descritto dalla funzione d'onda $\psi$, il valor medio dell'osservazione $\omega$, in una serie di misure, è uguale al valore atteso dell'operatore corrispondente $\hat{\Omega}$.
\end{quote}
Data una funzione d'onda $\psi$ ed un opertore $\hat{\Omega}$ il \index{valore atteso}\textbf{valore atteso} di un operatore è definito:
$$\hat{\left \langle \Omega \right \rangle} = \frac{ \int \psi^{\star} \hat{\Omega} \psi \,d\tau }{ \int \psi^{\star} \psi \,d\tau }$$
Scegliendo una \index{funzione d'onda normalizzata}\textbf{funzione d'onda normalizzata} ovvero una funzione che rispetti questa proprietà:
...
\begin{quote}
  \index{Postulato 3'}\textbf{Postulato 3'}: Quando $\psi$ è un'autofunzione dell'operatore $\hat{\Omega}$ la determinazione della proprietà $\hat{\Omega}$ fornisce sempre un unico risultato: l'autovalore corrispondente $\omega$. Quando $\psi$ non è un'autofunzione di $\hat{\Omega}$, una misura della proprietà fornisce un unico risultato, che coincide con uno degli autovalori di $\hat{\Omega}$, e la probabilità che si sia misurato un particolare autovalore $n$ è uguale a ${\left|c_n\right|}^2$, dove $c_n$ è il coefficente dell'autofunzione $\psi_n$ nello sviluppo della funzione d'onda $\psi$.
\end{quote}

\subsubsection{L'interpretazione della funzione d'onda}
Spesso chiamato \index{interpretazione di Born}\textbf{interpretazione di Born} il quarto postulato riguarda l'interpretazione della funzione d'onda.
\begin{quote}
  \index{Postulato 4}\textbf{Postulato 4}: La probabilità di rinvenire una particella nell'elemento di volume $d\tau$ al punto $r$ è proporzionale a ${\left|\psi(r)\right|}^2 d\psi$.
\end{quote}
Si chiamata \index{densità di probabilità}\textbf{densità di probabilità} la funzione ${\left|\psi(r)\right|}^2 d\psi$ perché moltiplicata per un certo volume, essa fornisce la probabilità di trovare la particella in quella regione. Nel caso monodimensionale:
$$\mathbb{P}(a, b) = \int_a^b {\left|\psi(x)\right|}^2 \, dx$$
La funzione $\psi$ viene anche detta \index{ampiezza di probabilità}\textbf{ampiezza di probabilità}, nell'esempio del caso monodimensionale si può notare il legame tra $\psi$ e l'ampiezza della funzione $\mathbb{P}$:
...
Il quinto ed ultimo postulato, anche noto come \index{equazione di Schr\"odinger}\textbf{equazione di Schr\"odinger}, espone come la funzione d'onda evolve nel tempo.
\begin{quote}
  \index{Postulato 5}\textbf{Postulato 5}: La funzione d'onda $\Psi(r_1, r_2, ..., t)$ evolve nel tempo secondo l'equazione
...
Quando l'energia potenziale è indipendente dal tempo, essa si può separare in funzioni legate al tempo e funzioni legate allo spazio. Applicando la tecnica della \index{separazione delle variabili}\textbf{separazione delle variabili} possiamo riscrivere $\Psi$:
