\lvlii{Oscillatore armonico}
\lvliii{Moto traslazionale}
...
$$p = \frac{h}{\lambda}$$
Quest'ultima relazione prende il nome di \idx{relazione di de Broglie}.

\lvliii{Densità di flusso}
...
dove $g(k)$ prenderà il nome di \idx{funzione peso} e $\Psi_k$ sarà la famiglia che conterrà le onde che sovrapponendosi formeranno $\Psi$.

\lvliii{Barriera di potenziale}
...
\lvliv{Parete di potenziale infinitamente spessa}
...
A questo punto, sapendo che la particella non può stare nella \textit{Zona II}, teniamo l'esponenziale che tende a $0$ ed eliminiamo l'altro imponendo $B' = 0$. Per quanto velocemente tenda a $0$ l'esponenziale, rimarrà comunque una piccola probabilità di rinvenire la particella all'interno della barriera, questo effetto prende il nome di \idx{penetrazione} e la distanza massima in cui si può trovare la particella all'interno della barriera è $\frac{1}{\kappa}$ e si chiama \idx{profondità di penetrazione}.

\lvliv{Parete di potenziale di spessore finito}
...
La \idx{probabilità di riflessione} è data dal rapporto tra la densità di flusso riflessa e la densità di flusso incidente:
$$R = \frac{|B|^2}{|A|^2}$$
Analogamente la \idx{probabilità di trasmissione} è data dal rapporto tra densità di fusso trasmessa e densità di flusso incidente:
$$P = \frac{|A''|^2}{|A|^2}$$
Si fa notare che $P = 1 - R$. Nonostante $E < V$ la particella può avere una $P \ne 0$ ovvero avere una certa probabilità di attraversare la barriera avendo comunque un energia inferiore ad essa, questa caratteristica prende il nome di \idx{effetto tunnel}.

\lvliii{Particella nella scatola}
Prendiamo in considerazione il problema di una particella vincolata a muoversi in una \idx{buca quadrata unidimensionale}, dividiamo quindi lo spazio in tre zone: nella prima $x < 0$ e nella terza zona $L < x$ il potenziale è $+\infty$ mentre vale $0$ nella rimanente $0 < x < L$. L'hamiltoniana del sistema è:
$$H=\begin{cases}
...
con condiziondi di contorno $\psi(0) = 0$ e $\psi(L) = 0$. Applicando la prima condizione otteniamo $\psi(0) = C$ e quindi risulta che $C = 0$. Applicano la seconda condizione abbiamo $\psi(L) = D \sin(kL) = 0$ dove escludiamo $D = 0$ perché soluzione banale e porterebbe la funzione d'onda ad essere nulla. Rimane come soluzione annullare la funzione seno imponendo $k = \frac{n\pi}{L}$ con $n = 1, 2, ...$. Il numero intero $n$ prende il nome di \idx{numero quantico}. Rimane da esprimere l'energia $E = \frac{k^2 \hbar^2}{2m}$ in funzione di $n$ come:
$$E_n = \frac{n^2 \hbar^2 \pi^2}{2 m L^2} = \frac{n^2 h^2}{8 m L^2} \qquad con \qquad n = 1, 2, ...$$
Da questa ultima formula si nota che l'energia non è libera di scegliere tutti i valori ma solo un sottoinsieme che dipende da $n$, questa caratteristica si chiama \idx{quantizzazione}.

\lvliii{La soluzione dell'oscillatore}
Un oscillatore è costituito da una particella che si muove periodicamente attorno ad una posizione di equilibrio soggetto da una forza di richimo. Nell'oscillatore armonico unidimensionale la forza di richiamo è $F(x) = -kx$ dove $k$ prende il nome di \idx{costante di forza}. Sapendo che il potenziale è legato alla forza mediante la formula $F(x) = -\partial_x V$ possiamo ricavarci la sua funzione:
$$V(x) = \frac{1}{2}kx^2$$
Due cose che si possono notare dalla formula sono: primo il \idx{potenziale parabolico} è simmertico e il suo asse di simmetria è nel punto $x = 0$, secondo il punto di simmetria è anche il punto di minimo del potenziale e corrisponde a $V(0) = 0$.
...
Introduciamo ora due nuovi operatori l'\idx{operatore di creazione} $\hat{a}^{\dagger}$ e l'\idx{operatore di annichilimento} $\hat{a}$:
...
dove $H_n$ sono dei polinomi particolari chiamati \idx{polinomi di Hermite}.
